...
import { renderMarkdown, parseMessageSegments, handleAssistantLinkClick } from './utils';

const SELECTED_MODEL_LS_KEY = 'gridman.ai.chat.selectedModel';
const AUTO_MODEL_ID = '';

const SIDEBAR_WIDTH_LS_KEY = 'gridman.ai.chat.sidebarWidth';
const DEFAULT_SIDEBAR_WIDTH = 420;
const MIN_SIDEBAR_WIDTH = 340;
const MAX_SIDEBAR_WIDTH = 720;

const clampSidebarWidth = (value) => Math.min(MAX_SIDEBAR_WIDTH, Math.max(MIN_SIDEBAR_WIDTH, value));

// Switching the active tab remounts this component. Keep the unsent draft here
// so a half-typed prompt survives the remount.
let draftInputCache = '';
...
const HistoryPopover = ({ items, activeId, onPick, onDelete, onClose }) => {
  ...
  useEffect(() => {
    ...
    // ownerDocument: in the popped-out window the popover lives in a different
    // document than the one this module closed over.
 ...
 * The AI chat panel.
 *
 * Lives to the right of the request pane and reads the collection tree only —
 * it never touches the collection sidebar's own rendering path. Mount it via
 * `AiChatSidebar/Surface`, which handles the ai.enabled gate.
 *
 * NO POPOUT IN THIS PASS. A "open in new window" button shipped in the port
 * and could not work: it called `window.open('', 'gridman-ai-assistant-N', …)`
 * and the main process's handler in packages/bruno-electron/src/index.js does
 *
 *     mainWindow.webContents.setWindowOpenHandler(({ url }) => { …; return { action: 'deny' }; });
 *
 * — it destructures `url` only (never `frameName`) and denies unconditionally,
 * so no `window.open` from the renderer has ever produced a window. To bring
 * the popout back the main-process owner must:
 *   1. destructure `frameName` alongside `url`;
 *   2. return `{ action: 'allow', overrideBrowserWindowOptions: {…} }` for
 *      frameNames starting with `gridman-ai-assistant`, with `nodeIntegration:
 *      false` and `contextIsolation: true`, and keep denying everything else;
...
const AiChatSidebar = ({ collection }) => {
  ...
  const [selectedModel, setSelectedModel] = useState(() => {
    try {
      return localStorage.getItem(SELECTED_MODEL_LS_KEY) ?? AUTO_MODEL_ID;
    } catch {
  ...
  const isOpen = useSelector((state) => state.ai.isOpen);
  const allChats = useSelector((state) => state.ai.chats);
  const tabs = useSelector((state) => state.tabs.tabs);
  const activeTabUid = useSelector((state) => state.tabs.activeTabUid);
  const preferences = useSelector((state) => state.app.preferences);
  const aiEnabled = get(preferences, 'ai.enabled', false);
  const redactVariables = get(preferences, 'ai.security.redactVariables', true);
  const preferredModel = get(preferences, 'ai.defaultModel', '');
  ...
  const aiContext = useMemo(() => {
    ...
    // Anything else (collection settings, runner, variables, …) falls back to
  ...
  // backend pick, so users get defaults that adapt as providers change.
  useEffect(() => {
    ...
    try {
      localStorage.setItem(SELECTED_MODEL_LS_KEY, AUTO_MODEL_ID);
    } catch {}
  ...
  const requestMethod = useMemo(() => {
    ...
    return activeItem.draft ? get(activeItem, 'draft.request.method', 'GET') : get(activeItem, 'request.method', 'GET');
  ...
  const contentType = useMemo(() => {
    ...
    switch (requestPaneTab) {
      case 'tests':
        return 'tests';
      case 'script':
        return scriptPaneTab === 'post-response' ? 'post-response' : 'pre-request';
      case 'docs':
        return 'docs';
      // Panes with no code surface of their own (Body, Headers, Auth, …) target
      // docs — every item has one, so the diff always has somewhere to land.
      default:
  ...
  // Bind the chat to the active context's pathname so the history list reflects
  // this specific request/folder/collection and persistence keys stay stable
  // across sessions.
  ...
  // First-open restore: if this tab has no conversation yet and there's a saved
  // one for the same file, load the most recent. Once per tab — an explicit
  // "new chat" is never auto-replaced.
  useEffect(() => {
  ...
  // request items. Folder and collection chats skip it.
  //
  // Built by the slice, never inline: `buildAiRequestContext` is where the URL
  // is redacted and the body is dropped when it cannot be redacted. A local
  // copy of this object was how the unredacted URL got out in the first place.
  const requestContext = useMemo(() => {
    if (aiContext?.kind !== 'request' || !isItemARequest(aiContext.item)) return null;
    return buildAiRequestContext(aiContext.item);
  }, [aiContext]);

  // Collection-scoped, so it works for request, folder and collection chats
  // alike. Each entry is { name, value, scope, secret }; anything marked secret
  // is already replaced with a placeholder here, before it can reach IPC.
  const aiVariables = useMemo(() => {
    if (aiContext?.kind === 'request') return buildAiVariablesPayload(collection, aiContext.item, redactVariables);
    if (aiContext?.kind === 'folder') return buildAiVariablesPayload(collection, aiContext.folder, redactVariables);
    return buildAiVariablesPayload(collection, null, redactVariables);
  ...
  // Re-measure the textarea on mount when a draft was restored from the module
  // cache (pop-out/dock remount) so it isn't stuck at one row.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e) => {
      ...
      setSidebarWidth((width) => {
        try {
          localStorage.setItem(SIDEBAR_WIDTH_LS_KEY, String(width));
        } catch {}
        return width;
      });
    };
  ...
  const handleSubmit = async (e) => {
    ...
    // Explicit picker wins; otherwise fall back to the pinned default from
    // Preferences, then to the backend's own choice.
    const modelForSend = selectedModel || preferredModel || AUTO_MODEL_ID;
    ...
    try {
      ...
      // Deliberately message-free. The provider's error text is attacker- (or
      // misconfiguration-) controlled and routinely echoes fragments of the
      // request or response back at us; console output ends up in devtools and
      // in crash/log captures. The user still sees the message in the panel
      // via setAiError, where it stays inside the app.
      console.error('Failed to send AI message (provider message withheld from logs)');
      setProcessingStage(null);
    }
  };
  ...
  const handleApplyCode = (code, originalCode, messageIndex, msgContentType, writeIndex) => {
    ...
    // DiffView already disables the button in this case, but guarding here too
    // means the keyboard / programmatic path can't blow away local edits.
    ...
    if (aiContext.kind === 'request') {
      ...
      switch (targetType) {
        case 'tests':
          dispatch(updateFolderTests({ ...payload, tests: code }));
          break;
        case 'pre-request':
          dispatch(updateFolderRequestScript({ ...payload, script: code }));
          break;
        case 'post-response':
          dispatch(updateFolderResponseScript({ ...payload, script: code }));
          break;
        case 'docs':
          dispatch(updateFolderDocs({ ...payload, docs: code }));
          break;
        // Bail rather than marking the diff accepted when nothing was
        // dispatched.
        default:
          return;
      }
      ...
      switch (targetType) {
        case 'tests':
          dispatch(updateCollectionTests({ ...payload, tests: code }));
          break;
        case 'pre-request':
          dispatch(updateCollectionRequestScript({ ...payload, script: code }));
          break;
        case 'post-response':
          dispatch(updateCollectionResponseScript({ ...payload, script: code }));
          break;
        case 'docs':
          dispatch(updateCollectionDocs({ ...payload, docs: code }));
          break;
        default:
          return;
      }
  ...
  const handleClose = () => dispatch(closeAiPanel());
  ...
  const selectedModelLabel = useMemo(() => {
    if (selectedModel === AUTO_MODEL_ID) {
      const pinned = availableModels.find((m) => m.id === preferredModel);
      return pinned ? `${pinned.label} (default)` : 'Auto';
    }
    return availableModels.find((m) => m.id === selectedModel)?.label || 'Auto';
  }, [availableModels, selectedModel, preferredModel]);
                    ...
                    {segments.map((segment, segIndex) => {
                      ...
                      return (
                        <div key={`p${partIndex}-s${segIndex}`} className="prose markdown-body">
                          {/* Assistant markdown is untrusted. Every click and
                              middle-click inside it is intercepted: without
                              this, an anchor with no target is a TOP-LEVEL
                              navigation and the app window leaves Gridman —
                              setWindowOpenHandler never sees that path. */}
                          <div
                            data-testid="ai-markdown-body"
                            onClick={handleAssistantLinkClick}
                            onAuxClick={handleAssistantLinkClick}
                ...
                && msg.writes.map((write, writeIdx) => {
                  ...
                  return (
                    <DiffView
                      ...
                      warning={
                        notRead
                          ? 'Content was not read first — changes may overwrite unrelated edits'
                          : isStale
                            ? 'Content has been modified since the model read it'
                            : null
                      }
  ...
  const renderEmptyState = () => {
    const suggestions = SUGGESTIONS_BY_TYPE[contentType] || SUGGESTIONS_BY_TYPE.docs;
    return (
      <div className="empty-state">
        ...
        <h3>Gridman AI</h3>
        <p>Ask me to generate or modify tests, scripts, and docs.</p>
        {/*
          Credentials in the request are redacted before anything leaves this
          machine; the content being EDITED is not. This used to name only Docs
          — but `allContent` ships Tests, Pre-Request and Post-Response scripts
          by the same route, so naming a subset here read as a promise about the
          rest. Every verbatim channel is listed, and the list matches
          OUTBOUND_VERBATIM_KEYS in providers/ReduxStore/slices/ai.js. Said here
          as well as in Preferences > AI > Security, because this is the screen
          someone is looking at when they press Send.
        */}
        <p className="empty-disclosure" data-testid="ai-docs-disclosure">
          Request credentials are redacted before sending. Your <strong>Docs</strong>, <strong>Tests</strong>,{' '}
          <strong>Pre-Request</strong> and <strong>Post-Response</strong> content is not — it is sent as written, along
          with the messages you type. See Preferences &gt; AI &gt; Security.
        </p>
  ...
  // Belt and braces: Surface already gates on this, but the panel must never
  // render (and therefore never poll status) with the feature switched off.
  if (!aiEnabled) return null;
  if (!isOpen) return null;
  if (!aiContext) return null;

  const placeholders = PLACEHOLDER_BY_TYPE[contentType] || PLACEHOLDER_BY_TYPE.docs;
        ...
        <div className="ai-sidebar-input">
          {availableModels.length === 0 ? (
            <div className="no-models-warning">
              No AI models available. Add an endpoint or a provider key in Preferences &gt; AI.
            </div>
          ) : (
            <div className="input-container">
              ...
              <div className="input-actions">
                ...
                {isLoading ? (
                  ...
                  <button className="send-btn" onClick={handleSubmit} title="Send (Enter)" disabled={!input.trim()}>
