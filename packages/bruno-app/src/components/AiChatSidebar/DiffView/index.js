...
const DiffView = ({ originalCode, newCode, onAccept, onReject, status, contentTypeLabel, warning, disableAccept }) => {
                ...
                <div className="expand-buttons">
                  {!isAtTop && (
                    <button
                      ...
                      onClick={() => expandUp(hunk.startIndex, hunk.count)}
                      title="Expand up"
                    >
                  ...
                  {!isAtBottom && (
                    <button
                      className="expand-btn"
                      onClick={() => expandDown(hunk.startIndex, hunk.count)}
                      title="Expand down"
                    >
  ...
  return (
    <StyledWrapper className={status || ''}>
      ...
      {warning && <div className={`diff-warning ${disableAccept ? 'error' : 'warn'}`}>{warning}</div>}
