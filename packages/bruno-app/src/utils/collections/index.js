...
const GENERIC_HOST_LABELS = new Set(['www', 'api', 'dev', 'stage', 'staging', 'qa', 'test', 'esb', 'gateway', 'gw']);
const GENERIC_PATH_SEGMENTS = new Set(['api', 'apis', 'rest']);
...
export const normalizeItemPathname = (pathname) => path.normalize(String(pathname || '').normalize('NFC')).replace(/\/+$/, '');
...
export const findParentItemInCollectionByPathname = (collection, pathname) => {
  ...
  return find(flattenedItems, (item) => {
    return item.items && find(item.items, (i) => normalizeItemPathname(i.pathname) === normalizedPathname);
  });
};
...
export const getTreePathFromCollectionToItem = (collection, _item) => {
  let path = [];
  // Guard against cycles: duplicate item uids can appear transiently while a
  // move/clone is being reconciled (watcher add + unlink races), which would
  // make the parent walk loop forever and freeze the renderer.
...
export const isAutoGeneratedRequestName = (name = '') => {
  return AUTO_REQUEST_NAME_PATTERN.test(String(name).trim());
};
...
const isGenericPathSegment = (segment = '') => {
  const lower = segment.toLowerCase();
  return GENERIC_PATH_SEGMENTS.has(lower) || /^v\d+$/i.test(lower);
};

const isIdentifierLikeSegment = (segment = '') => {
  const lower = segment.toLowerCase();
  return /^\d+$/.test(lower)
    || /^[0-9a-f]{16,}$/i.test(lower)
    || /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(lower);
};

const parseUrlForNaming = (url = '') => {
...
const getHostLabel = (parsedUrl) => {
  ...
  const meaningfulLabel = labels.find((label) => !GENERIC_HOST_LABELS.has(label)) || labels[0];
  return cleanLabel(meaningfulLabel);
};

const getPathLabel = (parsedUrl) => {
  const segments = (parsedUrl.pathname || '')
    .split('/')
    .map((segment) => cleanLabel(safeDecodeURIComponent(segment)))
    .filter((segment) => segment && !isGenericPathSegment(segment) && !isIdentifierLikeSegment(segment));

  return segments.slice(-2).join(' ');
};

export const deriveRequestNameFromUrl = (url = '') => {
  const parsedUrl = parseUrlForNaming(url);
  if (!parsedUrl) {
    return '';
  }

  const hostLabel = getHostLabel(parsedUrl);
  const pathLabel = getPathLabel(parsedUrl);
  const fallbackLabel = String(parsedUrl.hostname || '').replace(/^www\./i, '').trim();
  const name = pathLabel
    ? [hostLabel, pathLabel].filter(Boolean).join(' ')
    : (fallbackLabel || hostLabel);

  return name.replace(/\s+/g, ' ').slice(0, 80).trim();
};

export const deriveTransientRequestNameFromUrl = (currentName = '', url = '') => {
  if (!isAutoGeneratedRequestName(currentName)) {
    return '';
  }

  return deriveRequestNameFromUrl(url);
};
